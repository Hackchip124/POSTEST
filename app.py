...
def print_receipt(receipt_text):
    ...
    if platform.system() == "Windows" and printer_name and printer_name != "No printers found":
        try:
            import win32print
            import win32con
            ...
            st.error(f"Windows printing failed: {str(e)}")
...
class BarcodeScanner:
    ...
    def start_camera_scanning(self):
        ...
        while self.running and self.scanner.isOpened():
            ret, frame = self.scanner.read()
            if not ret:
                continue
            
            # Create a frame with margin and guide
            h, w = frame.shape[:2]
            
            # Draw a centered rectangle as scanning guide
            ...
            if self.scan_window:
                self.scan_window.image(frame, channels="BGR", use_column_width=True)
...
def setup_barcode_scanner():
    ...
    if scanner_type == 'serial':
        if barcode_scanner.init_serial_scanner(port):
            barcode_scanner.scanner_thread = threading.Thread(target=barcode_scanner.start_serial_scanning, daemon=True)
            barcode_scanner.scanner_thread.start()
    elif scanner_type == 'camera':
        if barcode_scanner.init_camera_scanner():
            barcode_scanner.scanner_thread = threading.Thread(target=barcode_scanner.start_camera_scanning, daemon=True)
            barcode_scanner.scanner_thread.start()
...
if 'pos_mode' not in st.session_state:  # 'scan' or 'manual'
...
def pos_scan_mode():
    ...
    if st.session_state.scanning_active:
        ...
        if not barcode_scanner.scanner:
            if not barcode_scanner.init_camera_scanner():
                st.session_state.scanning_active = False
                st.rerun()
        
        # Create a placeholder for the camera feed
        if barcode_scanner.scan_window is None:
            barcode_scanner.scan_window = st.empty()
        
        # Start scanning in a thread
        if not barcode_scanner.running:
            barcode_scanner.running = True
            barcode_scanner.scanner_thread = threading.Thread(target=barcode_scanner.start_camera_scanning, daemon=True)
            barcode_scanner.scanner_thread.start()
            ...
            for i in range(0, len(product_list), cols_per_row):
                ...
                for col_idx in range(cols_per_row):
                    if i + col_idx < len(product_list):
                        barcode, product = product_list[i + col_idx]
                        with cols[col_idx]:
                            # Product card with more details
                            with st.container():
                                # Display product image if available
                                if 'image' in product and os.path.exists(product['image']):
                                    try:
...
# Common cart and checkout display for both modes
def display_cart_and_checkout():
    settings = load_data(SETTINGS_FILE)
    
    st.header("Current Sale")
    if st.session_state.cart:
        # Display cart items with ability to modify quantities or remove
                ...
                if st.button("Process Return"):
                    ...
                    if not returned_items:
                        ...
                        if refund_method == "Cash":
                            # For cash refunds, we just need to record it
                            returns[return_id]['refund_method'] = "Cash"
                            returns[return_id]['status'] = "Completed"
                            
                            # Update cash drawer if in a shift
                            if is_cashier() and st.session_state.shift_started:
                        ...
                        else:  # Credit Card, Debit Card, Mobile Payment
    ...
    with tab2:
        ...
        with st.form("pos_config_form"):
            ...
            theme = st.selectbox(
                "Theme",
                ["Light", "Dark", "Blue"],
                index=["Light", "Dark", "Blue"].index(settings.get('theme', 'Dark'))
            )
